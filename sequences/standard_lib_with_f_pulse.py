...
def set_proteus_clock(clock_GSs):
    sclk = clock_GSs*1E9 
...
def rabi(): 
    ...
    # qubit pulses
    rabi_pulse = Pulse(start=readout_start-pi_time-2*b, duration=0, amplitude=1, ssm_freq=ssm, phase=0) 
    ...
    pi_ef = Pulse(start=readout_start-b-pi_time, duration=pi_time, amplitude=1, ssm_freq=ssm_ef, phase=0) 
    this_seq.add_sweep(channel=1, sweep_name='none', initial_pulse=pi_ef)
    pi_ef.phase = -1*mixer_orthogonality_deg
    this_seq.add_sweep(channel=2,  sweep_name='none', initial_pulse=pi_ef)
...
#    pi_time = 18
    T1_time = 10000
    readout_start = 125000
    readout_dur = 3600
    b = 5 # buffer
    
    # qubit pulses
    rabi_pulse = Pulse(start=readout_start-2*(pi_time+b), duration=pi_time, amplitude=1, ssm_freq=ssm, phase=0) 
...
def T2(use_echo=False, show_plot=True): #this is pulsed readout to ring up and ring down cavity dfor e state
    file_length = 65536 ## must be power of 2
    num_steps = 51  
    this_seq = Sequence(file_length, num_steps) #this creates something called rabi_seq that is an instance of a this_seq class
...
#    pi_time = 18
    ...
    # qubit pulses
    pi_ef_start = readout_start-b-pi_time
    fixed_pi2_start = pi_ef_start -2*(pi_time//2 - b)
    fixed_pi2_pulse = Pulse(start=fixed_pi2_start, duration=pi_time//2, amplitude=rabi_amp, ssm_freq=ssm, phase=0) 
    ...
    pi_ef = Pulse(start=pi_ef_start, duration=pi_time, amplitude=1, ssm_freq=ssm_ef, phase=0) 
    this_seq.add_sweep(channel=1, sweep_name='none', initial_pulse=pi_ef)
    pi_ef.phase = -1*mixer_orthogonality_deg
    this_seq.add_sweep(channel=2,  sweep_name='none', initial_pulse=pi_ef)
...
#    pi_time = 18
    ...
    # qubit pulses
    pi_ef_start = (readout_start -pi_time -b)
    fixed_pi2_start = pi_ef_start -pi_time//2 - b
    fixed_pi2_pulse = Pulse(start=fixed_pi2_start, duration=pi_time//2, amplitude=rabi_amp, ssm_freq=ssm, phase=0) 
...
def sweep_readout_amp(): 
    ...
    # readout pulse
    cavity_pulse = Pulse(start=readout_start, duration=3600, amplitude=0, phase=0) #pulse is also a class p is an instance
    start_amp, stop_amp = 0.7, 1.0
    this_seq.add_sweep(channel=4, sweep_name='amplitude', start=start_amp, stop=stop_amp,initial_pulse=cavity_pulse)
    print("Sweep amplitude from {start_amp*100}% to {stop_amp*100}%")
    ## markers
    alazar_trigger = Pulse(start=readout_start-1000, duration=500, amplitude=1)
    this_seq.add_sweep(channel=3, marker=1, sweep_name='none', initial_pulse=alazar_trigger )
...
def always_on(): 
    ...
    # readout pulse
    main_pulse = Pulse(start =0,duration = file_length, amplitude= readout_amp )
    this_seq.add_sweep(channel=4, sweep_name='none',initial_pulse=main_pulse)
...
def combined_seq():
    file_length = 131072
    num_steps_per_subset = 51
    this_seq = Sequence(file_length, num_steps_per_subset*2)

    ## constants   
    ssm = .100
    pi_time = 200
    T1_time = 110000
    readout_start = 120000
    rabi_time = 500
    b = 5 # buffer
    
    ## rabi section    
    for step_index, dur  in enumerate(np.linspace(0, rabi_time, num_steps_per_subset)):
        rabi_pulse = Pulse(start=readout_start-b, duration=-1*dur, amplitude=1, ssm_freq=ssm, phase=0) 
        this_seq.insert_bothChannels(1, rabi_pulse, step_index)
#    this_seq.add_sweep(channel=1, sweep_name='width', start=0, stop=-rabi_time,initial_pulse=rabi_pulse)
#    rabi_pulse.phase = mixer_orthogonality_deg
#    this_seq.add_sweep(channel=2,  sweep_name='width', start=0, stop=-rabi_time,initial_pulse=rabi_pulse)
        
    ## T1 section
    for step_index, delay in enumerate( np.linspace(0, T1_time, num_steps_per_subset)):
        step_index += 1*num_steps_per_subset
        pulse_start = readout_start-pi_time - delay
        pi_pulse = Pulse(start=pulse_start, duration=pi_time, amplitude=1, ssm_freq=ssm, phase=0)
        this_seq.insert_bothChannels(1, pi_pulse, step_index)
    print("last, ", step_index)
#    this_seq.add_sweep(channel=1, sweep_name='start', start=0, stop=-T1_time,initial_pulse=rabi_pulse)
#    rabi_pulse.phase = mixer_orthogonality_deg
#    this_seq.add_sweep(channel=2,  sweep_name='start', start=0, stop=-T1_time,initial_pulse=rabi_pulse)
#        
    ...
    ## Following pulses apply to all subsets
    # readout pulse
    readout_pulse = Pulse(start =readout_start,duration = 3600, amplitude= 1 )
    this_seq.add_sweep(channel=4, sweep_name='none',initial_pulse=readout_pulse)
    ...
    if True:
        seq_matrix = this_seq.channel_list[0][0] #[channel name -1][0:channel, 1:marker 1, 2:marker 2]
        plt.imshow(seq_matrix[0:200,int(110e3):readout_start+100], aspect='auto', extent=[110e3,readout_start+100,2*51,0])
        plt.show()
        
    ## write output
    #this_seq.load_sequence(inst_index=0, amp=[1.3,1.3,1,0.45] )
##END combined_seq
