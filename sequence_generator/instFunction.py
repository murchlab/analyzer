...
def loadSegmentData(inst, segNum, segData, query_err=False, verbose=True):
    SendScpi(inst, ":TRACe:DEF {0},{1}".format(segNum, len(segData)), verbose=verbose)
    SendScpi(inst, ":TRACe:SEL {0}".format(segNum), verbose=verbose)
...
def SendScpi(inst, line, query_err=False, verbose=True):
    try:
        if verbose:
            print(line)
        line = line + "\n"
