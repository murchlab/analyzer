...
def configure_board(alazar_params, board):
    ...
    board.inputControlEx(ats.CHANNEL_A,
                         ats.DC_COUPLING,
                         ats.INPUT_RANGE_PM_2_V, # JTM changed from 100 mV, 20/07/14 
...
#    generator.proteus_start_seq()
    ...
    try:
        ...
        while (buffersCompleted < buffers_per_acquisition and not ats.enter_pressed()):
            ...
            #print("\nb")
            board.waitAsyncBufferComplete(buffer.addr, timeout_ms=5000) #timeout_ms = 5000 earlier
            #print("\nc")
            buffersCompleted += 1
